...
class CRISPRlibQ:
	...
	### Create instance variables instead of class variables for class variables
	### are shared across all instances while instance variables are instance
	### specific. Those class variables which are set by immutable value, such
	### as numbers, strings and tuples, behave like instance specific. However,
	### this is not a correct practice since we really/actually need instance
	### variables here. Maybe using objects instead of dictionaries for jinja2
	### is not a correct practice either...
...
parser = argparse.ArgumentParser(description='Count CRISPR guides '
                                 'representation in NGS results of samples')
parser.add_argument('-i', '--input', help='Illumina fastq file(s)', nargs='+', 
                    required=True)
bt2index = parser.add_mutually_exclusive_group(required=True)
bt2index.add_argument('-t', '--table', help='Table with guide infomation. It '
                      'will be used for making bowtie2 index in the same '
					  'directory as this table. The table should have 3 '
					  'columns: 1) Gene name; 2) Guides name/number; 3) Guide '
					  'sequence. The first row will be discarded as header '
					  'which then followed by one guides per row. A fake header'
					  'is required if there is no header yet.', default=None)
bt2index.add_argument('--bt2', help='Bowtie2 index for expected guides.', 
                      default=None)
parser.add_argument('-r', '--randomer', help='Basepair position for randomer, '
                    'which will be used to remove PCR duplicates. Support both '
					'discrete and continuous regions. Discrete regions are '
					'separated by ",", and continuous region is expressed as '
					'"a:b".', default=None)
parser.add_argument('-g', '--guide', help='Basepair position for 20bp CRISPR '
                    'guide. Support both discrete and continuous regions. '
					'Discrete regions are separated by ",", and continuous '
					'region is expressed as "a:b".', default=None)
parser.add_argument('-o', '--outpath', help='Path to put outputs (with slash). '
                    'Default is the same directory as the first input fastq.', 
					default='')
parser.add_argument('-p', '--thread', help='Number of parallel search threads '
                    'for bowtie2 alignment. Default is 8.', default=8)
...
if not args.bt2:
	...
	bt2idxlog = os.path.join(bt2indexdir, '{}_build.log'.format(guidetab_base))
	fasta = os.path.join(bt2indexdir, guidetab_base + ".fasta")

	tab2fasta = ("tail -n +2 {} | awk -F'\\t' '{{print "
	             "\">\"$1\":\"$2\"\\n\"$3}}' > {}"
	...
	bt2build = "bowtie2-build {} {} 1>{} 2>&1".format(fasta, bt2index, 
	                                                  bt2idxlog)
	logging.info('Making Bowtie2 index from fasta file')
	logging.info(bt2build + '\n')
	logging.info('Check {} for progress and stats.'.format(bt2idxlog))
	os.system(bt2build)
...
for fastq in args.input:
	...
	guideseq = os.path.join(outtemp, name.replace(".fastq", 
	...
	bt2map_bc_sorted = os.path.join(outtemp, name.replace(".fastq", 
	                                                      "_bc_qnsorted.sam"))
	bt2map_samsort_log = os.path.join(outtemp, name.replace(".fastq", 
	                                                        "_bc_qnsorted.log"))
	bt2map_dedup = os.path.join(outdir, name.replace(".fastq", 
	                                                 "_qnsorted_dedup.sam"))
	dedup_stats = os.path.join(outdir, name.replace(".fastq", 
	                                                "_picard_dedup.txt"))
	dedup_log = os.path.join(outtemp, name.replace(".fastq", 
	                                               "_picard_dedup.log"))
	...
	ggfig = os.path.join(resultsum, name.replace(".fastq", 
	                                             "_guidespergene.png"))
	mggfig = os.path.join(resultsum, name.replace(".fastq", 
	                                              "_missguidespergene.png"))
	coverfig = os.path.join(resultsum, name.replace(".fastq", "_coverage.png"))

	### Extract molecular barcode (randomer) to name; Will be used by picard for
	### removing duplicates in sam file
	logging.info('Pre-processing fastq')
	yltk.process_fastq(fastq, bc_bp = args.randomer, slice_bp = args.guide, 
	                   output = guideseq)
	
	### Align to expected guides
	noMMalign = ("bowtie2 -p {} -L 20 -N 0 --no-1mm-upfront -x {} {} --un {} -S"
	            " {} > {} 2>&1".format(args.thread, bt2index, guideseq, 
				bt2unmap, bt2map, bt2stats))
	logging.info('Bowtie2 alignment')
	logging.info(noMMalign + '\n')
	os.system(noMMalign)
	
	### Attach barcodes to the BT&QT tags of samfile 
	### Remove duplicates using picard
	logging.info('Removing duplicates with picard')
	yltk.attach_barcode(bt2map, bt2map_bc)
	picardsort = ("java -Xmx2g -jar $PICARD SortSam I={} O={} "
	             "SORT_ORDER=queryname 2>{}".format(bt2map_bc, bt2map_bc_sorted,
				 bt2map_samsort_log))
	logging.info(picardsort)
	logging.info('Check {} for progress if this process is slow.'
	             ''.format(bt2map_samsort_log))
	os.system(picardsort)
	dedup = ("java -Xmx2g -jar $PICARD MarkDuplicates I={} O={} M={} "
	        "BARCODE_TAG=BC REMOVE_DUPLICATES=True 2>{}"
			"".format(bt2map_bc_sorted, bt2map_dedup, dedup_stats, dedup_log))
	logging.info(dedup + '\n')
	logging.info('Check {} for progress if this process is slow.'
	             .format(dedup_log))
	os.system(dedup)
	...
	countreads = ("samtools view -F 260 {} | cut -f 3 | sort | uniq -c | awk "
	             "'{{printf(\"%s\\t%s\\n\", $2, $1)}}' > {}"
				 .format(bt2map_dedup, gcounts))
	...
	os.system(findobs)
	countsum = ("awk -F':' '{{print $1}}' {} | sort | uniq -c | awk '{{print "
	           "$2\"\\t\"$1}}' > {}".format(obsguides, ctbygene))
	logging.info(countsum)
	os.system(countsum)	
	findexpect = ("samtools view -H {} | grep -P '@SQ\\tSN:' | awk -F'\\t' "
	             "'{{print substr($2,4)}}' | sort > {}".format(bt2map_dedup, 
				 expguides))
	logging.info(findexpect)
	os.system(findexpect)
	obsvsexp = ("comm -13 --check-order {} {} > {}".format(obsguides, expguides,
	           missguides))
	logging.info(obsvsexp)
	os.system(obsvsexp)
	missum = ("awk -F':' '{{print $1}}' {} | sort | uniq -c | awk '{{print "
	         "$2\"\\t\"$1}}' > {}".format(missguides, missbygene))
	logging.info(missum + '\n')
	os.system(missum)	
	...
	# If unique values of reads per guide are only a few (<=10), 
	# use bar graph to show all real data instead of histogram
	...
	### Save checking results in the CRISPRlibQ class 
	### Report it later using jinja2 templating system
	...
	with open(dedup_stats, 'r') as picardstatsfile:
		for line in picardstatsfile:
			if (line.strip() == '## METRICS CLASS	picard.sam.'
			                   'DuplicationMetrics'):
				break
		for line in picardstatsfile:
...
reporttemp = (jinja2.Environment(loader=jinja2.FileSystemLoader(searchpath=
             os.path.dirname(os.path.abspath(__file__))))
			 .get_template('report_template.html'))
with open(os.path.join(resultsum, 'Report.html'), 'w') as reporthtml:
	reporthtml.write(reporttemp.render(date=
	                 str(datetime.datetime.today().date()), 
					 mainfolder = maindir, results=libResults))
logging.info('Please find your results and the aggregated report at {}'
             .format(maindir))
