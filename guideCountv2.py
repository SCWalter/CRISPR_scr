...
def main():
    """Pipeline for processing fastq sequencing results of a CRISPR screen experiment.
    
    WARNING: this pipeline was only tested under linux environment. It could work on MacOS platform but may or may not work on Windows.
    """
    ...
    bt2index.add_argument('-t', '--table', help='Table with guide infomation, which will be used for making bowtie2 index in the same directory as this table. The table should have 3 columns: 1) Gene name; 2) Guides name/number; 3) Guide sequence. The first row will be discarded as header which then followed by one guides per row. A fake header is required if there is no header yet. If a Bowtie2 index has already been built before, you can provide it through the "--bt2" option. This option will not be accepted if a Bowtie2 index is provided by the "--bt2" option.', default=None)
    ...
    for fastq in args.input:
        ...
        # Save checking results in the CRISPRLibSeqQC class and report it later using jinja2 templating system
